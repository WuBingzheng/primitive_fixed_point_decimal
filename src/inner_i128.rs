...
impl FpdecInner for i128 {
    ...
    fn calc_mul_div_exp(self, b: Self, i: usize, rounding: Rounding) -> Option<Self> {
        ...
        if self ^ b >= 0 {
            ...
            let q = div_exp_fast(self as u128, b as u128, extra as u128, exp, i)?;
            i128::try_from(q).ok()
        } else {
            let extra = match rounding {
            ...
            let q = div_exp_fast(self.unsigned_abs(), b.unsigned_abs(), extra as u128, exp, i)?;
    ...
    fn div_exp(self, _exp: Self, i: usize) -> Self {
        let q = self.unsigned_abs().div_exp(0, i) as i128;
        q * self.signum()
    }
}
...
impl FpdecInner for u128 {
    ...
    // See the GM_MAGICS's doc.
    fn div_exp(self, _exp: Self, i: usize) -> Self {
        // SAFETY: the exp have been read already.
        let magic = unsafe { GM_MAGICS.get_unchecked(i) };

        // n / d := (n + ((n * m) >> 128)) >> l
        let (high, _) = mul2(self, magic.0);

        // n + high may overflow, so we
        (high + ((self - high) >> 1)) >> (magic.1 - 1)
    }
}
...
fn div_exp_fast(n1: u128, n2: u128, extra: u128, exp: u128, i: usize) -> Option<u128> {
    ...
    // calc: final q
    let (_, carry) = m_low.overflowing_add(n_low);
    let q = m_high + n_high + carry as u128;

    // correction by remainder
    // check: n - q * exp < exp
    let (pp_high, pp_low) = mul2(q, exp);
    let (r_low, borrow) = n_low.overflowing_sub(pp_low);
    let (r_high, _) = n_high.borrowing_sub(pp_high, borrow);

    if r_high == 0 && r_low < exp {
        Some(q)
    } else {
        Some(q + 1)
    }
    ...
    fn do_test_div_exp_fast(diff: u128) {
        for i in 1..39 {
            ...
            for j in 0..100000 {
                ...
                let q = n.div_exp(0, i as usize);
                assert_eq!(q, n / exp);

                let exp = exp as i128;

                let n = i128::MAX - (j * diff) as i128;
                let q = n.div_exp(0, i as usize);
                assert_eq!(q, n / exp);
